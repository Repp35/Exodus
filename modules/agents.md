...
2. **Sem acoplamento reverso.** Módulos não conhecem a estrutura do
   `init`. `init` carrega, chama `setup` de cada módulo, e sai. Se o
   `init` chama algo do módulo no heartbeat, está errado — empurra pro
   próprio módulo.
...
6. **Heartbeat fica onde o estado vive.** `AutoClash` registra o próprio
   `Heartbeat:Connect` no `setup` e desliga no `detachCharacter`. `init`
   não faz loop de personagem pra ninguém.
